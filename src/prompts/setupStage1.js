export function setupStage1Prompt(roomName) {
  return `你是一位专业的家居收纳顾问。这是用户家中「${roomName}」的照片。

请仔细分析照片中的收纳现状，识别所有可见区域和物品，返回以下 JSON 格式（只返回 JSON，不要包含任何其他文字或 markdown 代码块标记）：
...
重要：所有字符串值内部禁止使用英文双引号 " 作为强调或引用标记（会破坏 JSON 格式）。如需强调词语，请使用「」代替，例如「临时居住」而非 "临时居住"。`
}
